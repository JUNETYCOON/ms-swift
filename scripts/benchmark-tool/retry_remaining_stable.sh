...
LOG=/mnt/workspace/post-egoplan-stable-retry-gpu0.log
...
run_queue() {
    local benchmark=$1
    printf '[%s] starting stable retry: %s\n' "$(timestamp)" "${benchmark}" >> "${LOG}"
    EVAL_CUDA_VISIBLE_DEVICES=0 \
        PREPARED_ROOT_OVERRIDE=/tmp/stage1-official-prepared \
        SKIP_PREPARE=1 \
        OPENEQA_BATCH_SIZE=1 \
        OPENEQA_GROUP_VIDEO_BATCHES=0 \
        FLICKR30K_BATCH_SIZE=8 \
