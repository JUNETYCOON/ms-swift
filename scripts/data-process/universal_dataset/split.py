...
def _canonical_uri_identity(uri: str, input_path: Path) -> str:
    uri = uri.strip()
    ...
    if parsed.scheme.lower() == "file" and parsed.netloc.lower() not in ("", "localhost"):
        canonical = urlunsplit(
            ("file", parsed.netloc.lower(), parsed.path or "/", parsed.query, "")
        )
        return "uri:" + canonical
...
def _canonical_source_identity(value: Any, asset_id: Any) -> str:
    if not isinstance(value, Mapping):
        raise ValueError("Asset {!r} source identity must be an object".format(asset_id))
    namespace = value.get("namespace")
    source_value = value.get("value")
    if not isinstance(namespace, str) or not namespace.strip():
        raise ValueError("Asset {!r} source identity has no namespace".format(asset_id))
    if isinstance(source_value, bool) or not isinstance(source_value, (str, int)):
        raise ValueError("Asset {!r} source identity value must be a string or integer".format(asset_id))
    if isinstance(source_value, str) and not source_value.strip():
        raise ValueError("Asset {!r} source identity value must not be empty".format(asset_id))
    payload = [
        namespace.strip(),
        "integer" if isinstance(source_value, int) else "string",
        source_value,
    ]
    return "source-id:" + json.dumps(payload, ensure_ascii=True, separators=(",", ":"))


def _canonical_source_ref_identity(
    value: Any, asset_id: Any, input_path: Path
) -> Tuple[str, Optional[str]]:
    if not isinstance(value, Mapping):
        raise ValueError("Asset {!r} source_ref must be an object".format(asset_id))
    uri = value.get("uri")
    if not isinstance(uri, str) or not uri.strip():
        raise ValueError("Asset {!r} source_ref has no non-empty uri".format(asset_id))

    selectors: List[List[Any]] = []
    if "key" in value:
        key = value["key"]
        if isinstance(key, bool) or not isinstance(key, (str, int)):
            raise ValueError("Asset {!r} source_ref key must be a string or integer".format(asset_id))
        selectors.append(["key", "integer" if isinstance(key, int) else "string", key])
    for field in ("offset", "length"):
        if field not in value:
            continue
        selector_value = value[field]
        if (
            isinstance(selector_value, bool)
            or not isinstance(selector_value, int)
            or selector_value < 0
        ):
            raise ValueError(
                "Asset {!r} source_ref {} must be a non-negative integer".format(
                    asset_id, field
                )
            )
        selectors.append([field, selector_value])

    uri_identity = _canonical_uri_identity(uri, input_path)
    if selectors:
        reference_identity = "source-ref:" + json.dumps(
            [uri_identity, selectors], ensure_ascii=True, separators=(",", ":")
        )
    else:
        reference_identity = uri_identity

    sha256 = value.get("sha256")
    sha_identity: Optional[str] = None
    if isinstance(sha256, str) and sha256:
        if re.fullmatch(r"[A-Fa-f0-9]{64}", sha256) is None:
            raise ValueError(
                "Asset {!r} source_ref has an invalid sha256 value".format(asset_id)
            )
        sha_identity = "sha256:" + sha256.lower()
    return reference_identity, sha_identity


def canonical_asset_identities(asset: Mapping[str, Any], input_path: Path) -> Tuple[str, ...]:
    """Return every stable identity needed to connect one asset across records."""
    ...
    if isinstance(sha256, str) and sha256:
        ...
        identities.append("sha256:" + sha256.lower())
    source_ref = asset.get("source_ref")
    reference_identity: Optional[str] = None
    if source_ref is not None:
        reference_identity, reference_sha_identity = _canonical_source_ref_identity(
            source_ref, asset.get("id"), input_path
        )
        if reference_sha_identity is not None:
            identities.append(reference_sha_identity)
    source_identities = asset.get("source_identities") or []
    if not isinstance(source_identities, list):
        raise ValueError("Asset {!r} source_identities must be an array".format(asset.get("id")))
    for source_identity in source_identities:
        if not isinstance(source_identity, Mapping) or source_identity.get("verified") is not True:
            continue
        identities.append(_canonical_source_identity(source_identity, asset.get("id")))
    uri = asset.get("uri")
    if isinstance(uri, str) and uri.strip():
        identities.append(_canonical_uri_identity(uri, input_path))
    if reference_identity is not None:
        identities.append(reference_identity)
    if not identities:
        raise ValueError("Asset {!r} has no stable identity".format(asset.get("id")))
    return tuple(dict.fromkeys(identities))


def canonical_asset_identity(asset: Mapping[str, Any], input_path: Path) -> str:
    """Return the preferred identity while preserving the legacy public API."""

    return canonical_asset_identities(asset, input_path)[0]


class AssetIdentityRegistry:
    """Track stable aliases and enforce one content hash per connected component."""

    def __init__(self) -> None:
        self._aliases = _DisjointSet()
        self._component_sha256: Dict[str, str] = {}

    def add(self, asset: Mapping[str, Any], input_path: Path) -> Tuple[str, ...]:
        identities = canonical_asset_identities(asset, input_path)
        existing_roots = {
            self._aliases.find(identity)
            for identity in identities
            if identity in self._aliases.parent
        }
        component_hashes = {
            self._component_sha256[root]
            for root in existing_roots
            if root in self._component_sha256
        }
        component_hashes.update(
            identity for identity in identities if identity.startswith("sha256:")
        )
        if len(component_hashes) > 1:
            raise ValueError(
                "Asset {!r} reuses one stable alias with conflicting sha256 values".format(
                    asset.get("id")
                )
            )

        for identity in identities:
            self._aliases.add(identity)
        roots_before_union = {self._aliases.find(identity) for identity in identities}
        for identity in identities[1:]:
            self._aliases.union(identities[0], identity)
        for root in roots_before_union:
            self._component_sha256.pop(root, None)
        if component_hashes:
            self._component_sha256[self._aliases.find(identities[0])] = next(
                iter(component_hashes)
            )
        return identities

    def __len__(self) -> int:
        return len({self._aliases.find(identity) for identity in self._aliases.parent})
...
def _commit_pair(
    ...
    pairs = ((train_temporary, train_output), (val_temporary, val_output))
    if not overwrite:
        linked: List[Tuple[Path, Path]] = []
        try:
            for temporary, output in pairs:
                os.link(str(temporary), str(output))
                linked.append((temporary, output))
        except BaseException:
            for temporary, output in linked:
                try:
                    if os.path.samefile(str(temporary), str(output)):
                        output.unlink()
    ...
    for line_number, record in iter_jsonl(input_path):
        ...
        if audit_assets:
            for asset in record.get("assets") or []:
                if not isinstance(asset, Mapping):
                    continue
                for identity in asset_identities.add(asset, input_path):
        ...
        if isinstance(episode, Mapping):
            ...
            if isinstance(episode_id, str) and episode_id:
                previous = episode_groups.setdefault(episode_id, group_id)
                if previous != group_id:
                    groups.union(previous, group_id)
        total_records += 1
    if total_records == 0:
