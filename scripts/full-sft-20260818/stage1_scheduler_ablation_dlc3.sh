...
ABLATION_ROOT=${ABLATION_ROOT:-${STAGE_ROOT}/tmp/scheduler_ablation_20260813}
SAMPLE_PER_DATASET=${SAMPLE_PER_DATASET:-256}
...
case "${DLC_RANK}" in
  0)
    EXP_NAME=rank0_linear_lr1e-5
    LR_SCHEDULER_TYPE=linear
    LEARNING_RATE=1e-5
    ;;
  1)
    EXP_NAME=rank1_cosine_lr1e-5
    LR_SCHEDULER_TYPE=cosine
    LEARNING_RATE=1e-5
    ;;
  2)
    EXP_NAME=rank2_cosine_lr5e-6
    LR_SCHEDULER_TYPE=cosine
    LEARNING_RATE=5e-6
    ;;
  *)
...
SAMPLED_DIR=${ABLATION_ROOT}/sampled_jsonl_seed${SAMPLE_SEED}_n${SAMPLE_PER_DATASET}
SAMPLED_MANIFEST=${ABLATION_ROOT}/sampled_manifest_seed${SAMPLE_SEED}_n${SAMPLE_PER_DATASET}.json
READY_MARK=${ABLATION_ROOT}/sampled_manifest_seed${SAMPLE_SEED}_n${SAMPLE_PER_DATASET}.ready
OUTPUT_DIR=${OUTPUT_DIR:-${STAGE_ROOT}/sft-model/scheduler-ablation-20260813/${EXP_NAME}}
...
export MASTER_PORT=$((29510 + DLC_RANK))

# Prevent DLC's cross-node rank variables from leaking into the local-only test.
...
mkdir -p "${ABLATION_ROOT}" "${LOG_DIR}" "${OUTPUT_DIR}"
...
if [[ "${DLC_RANK}" == "0" ]]; then
  rm -f "${READY_MARK}"
  python3 - "${SOURCE_MANIFEST}" "${SAMPLED_DIR}" "${SAMPLED_MANIFEST}" "${SAMPLE_PER_DATASET}" "${SAMPLE_SEED}" <<'PY'
import json
import random
import sys
...
for name, cfg in manifest.get("datasets", {}).items():
    if not cfg.get("enabled", True):
        out["datasets"][name] = cfg
        out["datasets"][name]["enabled"] = False
        continue
...
PY
  ...
  echo "Waiting for sampled manifest from rank0..."
  for _ in $(seq 1 1800); do
    [[ -f "${READY_MARK}" ]] && break
    sleep 2
  done
  if [[ ! -f "${READY_MARK}" ]]; then
    echo "Timed out waiting for ${READY_MARK}" >&2
    exit 3
  fi
fi
...
swift sft \
  ...
  --run_name "scheduler-ablation-${EXP_NAME}" \
