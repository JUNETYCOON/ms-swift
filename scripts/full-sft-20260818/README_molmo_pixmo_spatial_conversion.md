...
```bash
python /mnt/luojunkun/stage1/ms-swift/scripts/data-process/prepare_molmo_pixmo_spatial_swift.py \
  --datasets all \
  --num-workers 8 \
  --max-source-rows 200 \
...
```bash
python /mnt/luojunkun/stage1/ms-swift/scripts/data-process/materialize_molmo2_videotrack_media.py \
...
```bash
python /mnt/luojunkun/stage1/ms-swift/scripts/data-process/audit_molmo2_videotrack_swift.py \
...
For a partial-image training run, rebuild both the ordinary local splits and
the already globally deduplicated training split after each download batch:
...
python scripts/data-process/download_pixmo_media.py \
  ...
  --rewrite-splits train val global_train \
  --training-manifest \
    /mnt/luojunkun/stage1/dataset_ms-swift/pixmo_partial_training_manifest.json
```

This writes `local_train.jsonl`, `local_val.jsonl`, and
`local_global_train.jsonl`. Use `local_global_train.jsonl` for training so the
existing cross-dataset deduplication is retained. Missing media remains listed
in the matching `local_*_rejected.jsonl`; rerunning the command after more
downloads atomically expands the local files without changing the source JSONL.
...
swift sft \
  ...
  --dataset /mnt/luojunkun/stage1/dataset_ms-swift/pixmo-points/local_global_train.jsonl \
