...
def _render_conversation(
    ...
    for asset_id in asset_occurrences:
        ...
        media_values[key].append(_resolve_media(str(asset["uri"]), base_dir))
    ...
    for message_index, message in enumerate(messages):
        ...
        output_messages.append({"role": str(role), "content": "".join(chunks)})
    ...
    result: Dict[str, Any] = {"messages": output_messages}
    if include_ids:
        result["id"] = str(record["id"])
        result["group_id"] = str(record["group_id"])
...
def _annotation_conversation(
    ...
    if annotation_type == "caption":
        ...
        messages = [
            ...
            {"role": "assistant", "content": [{"type": "text", "text": str(annotation.get("text", ""))}]},
        ]
    elif annotation_type == "qa":
        question = annotation.get("question")
        question_content = question if isinstance(question, list) else [{"type": "text", "text": str(question or "")}]
        answer = _select_answer(annotation, answer_policy)
        answer_content = answer.get("content")
        if not isinstance(answer_content, list):
            answer_content = [{"type": "text", "text": str(answer.get("text", ""))}]
        messages = [
...
def ms_swift_projection_warnings(
    ...
    for annotation in selected_annotations:
        if not isinstance(annotation, Mapping) or annotation.get("type") != "qa":
            continue
        answers = annotation.get("answers") or []
        if len(answers) > 1:
            warnings.append(
